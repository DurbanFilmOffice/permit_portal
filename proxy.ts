...
export default auth((req) => {
  ...
  if (isPublicPath) {
    if (isLoggedIn) {
      return NextResponse.redirect(
        new URL(getDashboardUrl(role), nextUrl)
      )
    }
    return NextResponse.next()
  }
  ...
  if (
    ...
    !(['admin', 'super_admin'] as Role[]).includes(role as Role)
  ) {
    return NextResponse.redirect(
      new URL('/admin/dashboard', nextUrl)
    )
  ...
  const internalRoles: Role[] = [
    'external_user', 'permit_officer', 'permit_admin',
    'admin', 'super_admin',
  ]
  if (
    path.startsWith('/admin') &&
    !internalRoles.includes(role as Role)
  ) {
